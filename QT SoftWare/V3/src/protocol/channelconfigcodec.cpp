 ...
 *
 * ── 与 V2 的差异 ──
 *   V2 依赖 utils/byteutils.h 的 ByteUtils::appendUInt32BE / readUInt32BE；
 *   V3 没有 utils 库，故把这两个大端读写辅助函数内联到本文件的匿名命名空间，
 *   作为私有实现（不得创建 utils 目录），对外接口保持不变。
 *
 * ── WPF 对照 ──
 *   float ↔ 大端 uint32 的转换用 std::memcpy 保位模式（不改变浮点值），
 *   相当于 C# 的 BitConverter.GetBytes / ToSingle 配字节序翻转。
 */
...
namespace {
// ---------------------------------------------------------------------------
// 大端 4 字节读写辅助函数（内联私有实现，替代 V2 的 utils::ByteUtils）
// ---------------------------------------------------------------------------

/**
 * @brief 把 32 位无符号整数以大端序追加到字节数组末尾
 * @param out   目标数组（非 const 引用：函数会修改它）
 * @param value 要写入的值
 *
 * 大端 = 高字节在前：依次写入 bit31..24、bit23..16、bit15..8、bit7..0。
 */
...
/**
 * @brief 从字节数组大端序读取 32 位无符号整数
 * @param data   数据源（const 引用）
 * @param offset 起始字节下标（0 起）
 * @param out    出参引用：成功后写入读取值
 * @return 越界返回 false 且不修改 out；成功返回 true
 */
bool readUInt32BE(const QByteArray &data, int offset, quint32 &out)
{
    if (offset < 0 || offset + 4 > data.size())
        return false;

    out = (static_cast<quint32>(static_cast<quint8>(data.at(offset))) << 24)
...
QByteArray ChannelConfigCodec::encodeChannel(const ChannelConfigInfo &ch)
{
    ...
    appendUInt32BE(out, minBits);
    appendUInt32BE(out, maxBits);
...
bool ChannelConfigCodec::decodeChannel(const QByteArray &payload, int offset, ChannelConfigInfo &out)
{
    ...
    if (!readUInt32BE(payload, minStart, minBits)
        || !readUInt32BE(payload, minStart + 4, maxBits))
        return false;
